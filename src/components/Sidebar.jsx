...
const Sidebar = () => {
  ...
  return (
    <aside className="w-64 border-r border-zinc-100 bg-white dark:bg-[#060607] dark:border-white/5 flex flex-col pt-5 pb-8 h-screen sticky top-0 overflow-y-auto hidden md:flex">
          ...
          <div key={cat.name} className="space-y-1.5">
            ...
            {cat.tools.map(tool => (
              <Link key={tool.id} to={tool.path} className={`group flex w-full items-center gap-3 px-3 py-2.5 text-sm font-medium rounded-lg transition-colors ${isActive(tool.path) ? activeClass : defaultClass}`}>
                <span className="text-zinc-400 dark:text-zinc-600 font-medium group-hover:text-zinc-700 dark:group-hover:text-zinc-200">
